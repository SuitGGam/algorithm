...
public class BOJ15657 {
    ...
    static void combinationWithRepetition(int start, int depth) { // 중복조합을 만드는 함수
        // 종료 조건
        if (depth > M) {
        ...
        for (int i = start; i <= N; i++) { // 중복조합 만들기
            selected[depth] = number[i]; // 숫자 고르기
            combinationWithRepetition(i, depth + 1); // 다음 숫자 고르기
        }
    } // combinationWithRepetition 종료
    ...
    public static void main(String[] args) throws IOException {
        ...
        st = new StringTokenizer(br.readLine(), " ");
        for (int i = 1; i <= N; i++) {
            number[i] = Integer.parseInt(st.nextToken()); // 숫자 저장
        }
        ...
        combinationWithRepetition(1, 1); // 중복조합 만들기
