...
public class BOJ1238 {
    ...
    static void reset() { // X로 갔다 오는 소요 시간을 초기화 하는 함수
        for (int i = 1; i <= N; i++) {
            goXTime[i]    = Integer.MAX_VALUE;
        }
    } // reset 종료
    
    static void goX(int start) { // X로 가는 시간 구하는 함수
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 1; i <= N; i++) { // 각 마을에서 X로 출발
            if (i == X) continue; // X 출발하는 경우는 continue
